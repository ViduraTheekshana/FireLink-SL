...
const TrainingSessionManager = () => {
  ...
  const fetchStaffMembers = async () => {
    try {
      const res = await fetch("http://localhost:5006/sessions/staff-members");
  ...
  const fetchSessions = async () => {
    try {
      const res = await fetch("http://localhost:5006/sessions");
      const data = await res.json();
  ...
  try {
    const res = await fetch(`http://localhost:5006/attendance/${sessionId}`);
    const data = await res.json();
    if (data.status === "ok") {
  ...
  const handleDeleteSession = async (id) => {
    if (window.confirm("Are you sure you want to delete this session?")) {
      try {
        await fetch(`http://localhost:5006/sessions/${id}`, {
          method: "DELETE",
        });
